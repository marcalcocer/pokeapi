...
public class PokeApiClient {
    ...
    public List<String> getAllPokemonNames() {
        return webClient.get()
                ...
                .map(resp -> resp.getResults().stream().map(PokeApiListResponse.PokemonSummary::getName).collect(Collectors.toList()))
                .block();
    }

    public Pokemon getPokemonDetail(String name) {
        PokeApiPokemonDetailResponse dto = webClient.get()
